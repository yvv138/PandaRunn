...
int main()
{
    ...
    sf::Sprite panda(texture);
    int numFrames = 4;
    int currentFrame = 0;
    panda.setScale(0.1f, 0.1f);
    int frameWidth = texture.getSize().x / numFrames;
    int frameHeight = texture.getSize().y;
    panda.setTextureRect(sf::IntRect(0, 0, frameWidth, frameHeight));
    panda.setPosition(100, 298);
    ...
    while (window.isOpen())
    {
        ...
        if (elapsedTime >= totalAnimationTime)
        {
            ...
            if (!collision)
            {
                if (currentFrame < 3)
                {
                    panda.setTextureRect(sf::IntRect(currentFrame * frameWidth, 0, frameWidth, frameHeight));
                }
                else
                {
                    panda.setTextureRect(sf::IntRect(3 * frameWidth, 0, frameWidth, frameHeight));
                }
            }
            else
            {
        ...
        if (jump && !collision)
        {
            panda.move(0, -gravity);
            gravity -= 0.2f;
            if (panda.getPosition().y >= 298)
            {
                jump = false;
                gravity = 10.0f;
                panda.setPosition(panda.getPosition().x, 298);
            }
        }
        else if (!collision)
        {
        ...
        int x = window.getSize().y - panda.getGlobalBounds().height - 50;
        for (const auto& obstacle : obstacles) {
            if (panda.getGlobalBounds().intersects(obstacle.sprite.getGlobalBounds()) && !collision) {
                collision = true;
            }
        }
